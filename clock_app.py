...
def analog_weather_mini():
    ...
    panel_h = AN_PR_H - 74
    section_h = panel_h // 4
    lcd.fill_rect(AN_PR_X, y0, AN_PR_W, panel_h, BG)
    if not _weather_cache_days:
        for index in range(4):
            y = y0 + index * section_h
            lcd.fill_rect(AN_PR_X, y, AN_PR_W, section_h, DARKGRAY)
            lcd.rect(AN_PR_X, y, AN_PR_W, section_h, GRAY)
        lcd.text("HAVA --", AN_PR_X + 12, y0 + 7, GRAY, 1)
        return
    ...
    sections = (
        ("ISI", current + " " + hi + "/" + low),
        ("YAGMUR", "%" + _forecast_number(day.get("rain"))),
        ("RUZ/GUST", _forecast_number(day.get("wind")) + "/" +
         _forecast_number(day.get("gust"))),
        ("DOG/BAT", day.get("sunrise", "--:--") + "/" +
         day.get("sunset", "--:--")),
    )
    for index, (title, value) in enumerate(sections):
        ...
        lcd.fill_rect(AN_PR_X, y, AN_PR_W, section_h, DARKGRAY)
        lcd.rect(AN_PR_X, y, AN_PR_W, section_h, GRAY)
        lcd.text(title, AN_PR_X + (AN_PR_W - len(title) * 6) // 2,
                 y + 2, TITLE_COL, 1)
        if len(value) > 11:
            value = value[:11]
